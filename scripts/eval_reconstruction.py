...
def calculate_ssim(img1, img2, data_range=1.0):
    # img1 and img2 are 3D arrays (H, W, T)
    # We can pass channel_axis=None so it treats it as a true 3D volume, 
    # or iterate over frames. skimage handles 3D arrays natively.
...
def run_evaluation(filter_class, B_data, name, flux_noise_std=None):
    ...
    if hasattr(filts, 'response_and_edges'):
        ...
        runtime = rec_time_end - start_time
        edge_time = 0.0 # It is integrated into the runtime
        ...
        edge_strength = edges['strength']
        rec_time_end = time.time()
        edge_time = rec_time_end - rec_time_start
    ...
    # Scale edge strength to [0, 1] for SSIM
...
if __name__ == "__main__":
    ...
    # Get all .mat files in DATA_BASEDIR
    ...
    for data_file in data_files:
        ...
        try:
            ...
            # Normalize by M to get per-exposure counts (matching run_base.py)
            if M != 1:
                B = B * (1.0 / M)
            
            print(f"Data shape: {B.shape}, M={M}")
            
            # Estimate noise using ippd + wide smoothing (matching run_base.py)
            ...
            # (Log Gabor FFT wrap-around and IIR startup transients)
            N_SKIP = 15
            H, W, N = B.shape
            ...
            # F-Score with tolerance (treat Log Gabor as pseudo-GT)
            PERCENTILES = list(range(50, 100, 5))
